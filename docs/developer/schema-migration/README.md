...
- **Part 2 - Lazy Migration** (afterward, no second cutover). Stamp a version into each lock. For a genuine future algorithm change, "replay" the lock's own version to check whether its real inputs moved; if not, quietly re-stamp the lock with **no rebuild**. Locks migrate one at a time, only when touched for a real reason. *Part 2 is **deferred and its design is provisional** - it is not built at the reset, and its specifics are open to re-design when actually implemented; only the cheap Part 1 reservations that keep it sound are committed now.* See [Part 2 - Lazy Migration](part-2-lazy-migration.md).
