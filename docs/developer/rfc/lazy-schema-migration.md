...
| `int` (forward-looking) | base-10 | `IsZero` (`0`) | scalar leaf; no plain `int` is measured by `projectV1` today (`manual-bump` is a combiner input, not projected) |
...
const resolutionPrefixHandled = false // flip true once updateResolutionHash co-stamps the prefix
...
func init() {
    ...
    if maxRes > minSupportedLockContentVersion && !resolutionPrefixHandled {
        panic("resolution algorithm advanced past floor without prefix co-stamp; see ResolutionInputHash")
    }
}
```
