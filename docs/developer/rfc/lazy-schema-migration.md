...
| `int`, sized ints (`int8..int64`), `uint`/sized | base-10 | `IsZero` (`0`) | scalar leaf; no plain `int` is measured by `projectV1` today (`manual-bump` is a combiner input, not projected) |
...
| `[]Struct` (`[]ComponentOverlay`, `SourceFiles`) | each element via its frozen sub-projector, **in resolved slice order** | no element projects bytes | **composite slice**: element order is a *deliberate, frozen* semantic - reorder = different config; the resolver pins a deterministic order (config-authored order, not a post-merge accident) and a golden vector locks it |
| `map[string]string` (`defines`) | sorted-key `<len>:k=<len>:v` | no entries | key membership measured (`{"k":""}` ≠ `{}`) |
| `map[string]Struct` (`map[string]PackageConfig`) | sorted-key; value via frozen sub-projector | **no entry's sub-projector emits measured bytes** (projected emptiness, *not* key-membership) | **stays measured/in-graph**: emits nothing while every value projects empty (e.g. `PackageConfig` all-`-`), so it adds no bytes today yet the completeness walk still descends - a future build-effective `PackageConfig` field is auto-measured |
...
**Cost of pruning at `-`, and its tripwire (G5 guard).** Excluding a composite with `-` removes its subtree from the completeness walk, so a *future* build-effective field added under a pruned type would be **silently unmeasured**. The design's first defense is to **prune as little as possible**: a composite that is publish-only *today* but documented growable (`Packages`/`PackageConfig`) stays **measured** and is zeroed by projected emptiness (load-out item 7), so its subtree stays walked and a new build field is structurally caught by the mandatory-tag rule. For composites that genuinely must be pruned (a `-` edge with no measurable content ever, e.g. `Origin`/diagnostic-only structs), the residual blind spot is closed by a **generated structural inventory check** (PR-A deliverable): for every `-`-pruned composite, CI asserts its field set stays within a checked-in known-inert list, so adding any field there fails CI and forces re-evaluation of the parent exclusion. This covers **all** `-`-pruned composites, not just one - it replaces the retired `TestAllFingerprintedFieldsHaveDecision` reach with a generator-maintained equivalent.
...
Several invariants this RFC relies on are, on inspection, still *prose/discipline* in exactly the places that are irreversible after the PR-B one-way door. Each must become a real CI/compile/branch-protection guard before the reset ships. These are PR-A/PR-B acceptance criteria, not aspirations. **They are labelled by enforcement kind, so future maintainers calibrate trust correctly:** *compiler/type* (cannot be wrong and still build), *generator* (fails at `go generate`), *CI test* (fails a required status check), *branch protection* (fails review/merge). The `init()` tripwire, the named CI tests, and the ordering gate are **behavioural**, not structural - they fire at test time, so like any other test they live or die with the CI suite and code review.

1. **Historical-no-recompute import guard** (*CI test*; makes [G6](#goals) structural). The "no reader recomputes a historical fingerprint" invariant is prose today. Make it an import/package-boundary check: the historical-reader surface (anything reachable from `synthistory` / `lockfile.ShowAtCommit`) MUST NOT import or call `ComputeIdentity`/`ComputeIdentityAt`. A linter/architecture test fails the build if that edge ever appears.
...
3. **Pruned-composite structural inventory** (*generator*, see [prune cost](#baseline-v1-omit-if-zero-no-include-always-legacy)). Generated, covering **all** `-`-pruned composites (not just `PackageConfig`), replacing the retired `TestAllFingerprintedFieldsHaveDecision` reach - a PR-A deliverable.

## Consciously ratified trade-offs

These are *conscious, permanent* choices, not oversights - recorded so they are ratified rather than defaulted into, since each is irreversible after PR B:
...
- **Lazy-forever + floor cadence** is an optimistic operational commitment. Before code (not at first floor-raise): pin the CI version-spread **ceiling value** and a **floor-raise SLA + owner**, define the PR-F emergency path, and ship `component migrate --dry-run/--plan` plus a scheduled version-spread dashboard.
- **Diagnosability:** the opaque token (manifest foreclosed) makes a year-3 production hash mismatch an internals-only exercise. Ship a `component fingerprint explain` token-explainer with PR B/C.
...
- **`Parse` read-gate insurance:** the format-`Version == 1` *policy* is sound, but make the *read* gate `Version <= currentVersion` (writes stay exact-match). Behaviorally identical today; preserves the forward-read escape hatch that an exact-match gate would otherwise freeze shut for already-deployed binaries.
...
| Canonical byte encoding = existing length-prefixed `<len>:<key>=<len>:<value>`; maps sorted-key; per-type value slots - pinned irreversibly at the reset | [§Baseline v1: encoding table](#baseline-v1-omit-if-zero-no-include-always-legacy) |
| Frozen-ness = compiler + generator + regeneration-idempotence; golden-vector coverage (tag-independent dropped-fields oracle) is the backstop; exclusion ledger kept for `-` fields | [§Golden-vector coverage: the backstop](#golden-vector-coverage-the-backstop) |
