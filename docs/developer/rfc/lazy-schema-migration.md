...
- **Smaller blast radius.** `ResolutionInputHash` does **not** feed `synthistory`, so an algorithm change can never mint a phantom changelog/release (that hazard is fingerprint-only). Worst case is a one-line `resolution-input-hash` rewrite per lock plus a wasted re-resolution that usually yields the same commit. Churn, not corruption. (One honest caveat: a resolution-only bump moves the `input-fingerprint` *line bytes* too — the `v<N>:` prefix advances — on a component whose render inputs did not change. Digest-comparison immunizes consumers from a phantom release, but it slightly erodes the raw "this `.lock` line moved ⇒ inputs changed" reviewer signal; the *digest*, not the line, is the trustworthy signal post-reset.)
