...
```
  ...
  -s, --spec-path stringArray         Spec path
      --upstream-status string        only include entries whose metadata declares this upstream status ('upstreamed', 'upstreamable', 'needs-upstream-hook', 'inapplicable', or 'unknown')
```
