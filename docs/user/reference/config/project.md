...
| Rendered specs directory | `rendered-specs-dir` | string | No | Output directory for `component render` (relative to this config file; default: `SPECS`) |
...
- **`rendered-specs-dir`** — rendered spec and sidecar files are written here by `azldev component render` (defaults to `SPECS` if not set)
