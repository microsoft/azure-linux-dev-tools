...
For upstream components (spec type `upstream`), `azldev component update` captures the Version / Release tags from the upstream spec at the resolved commit and stores them in the component's lock file (`locks/<name>.lock`). At build time these values are exposed as RPM macros so downstream specs can reference the upstream identity without hardcoding it or shelling out to `rpmspec`.
...
For local and SRPM components (spec types `local` and `srpm`), these macros are not populated.
