...
func GenerateSchema() error {
	...
	if err != nil {
		return mageutil.PrintAndReturnError("Schema generation failed.", ErrSrcCode, err)
	}
	...
	if err != nil {
		return mageutil.PrintAndReturnError("Failed to write schema file.", ErrSrcCode, err)
	}
