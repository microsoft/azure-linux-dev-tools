...
// addUpstreamProvenanceMacros injects %azl_upstream_version / _release into the
// given macros map from the component's lock data. Empty fields are skipped so
// components with unparseable upstream specs don't get blank macros. Existing
// entries in macros (i.e., user-provided defines that happen to share the name)
// win — this function only adds; it doesn't overwrite.
