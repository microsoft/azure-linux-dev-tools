...
func (t TestDefinition) validatePytest(testName string) error {
	if len(t.Pytest) == 0 {
		return fmt.Errorf(
			"%w: test %#q of type %#q requires a [pytest] subtable",
			ErrMissingTestField,
			testName,
			t.Type,
		)
	...
	if len(t.Lisa) > 0 {
		return fmt.Errorf(
			"%w: test %#q of type %#q cannot include subtable 'lisa'",
	...
	if len(t.Tmt) > 0 {
		return fmt.Errorf(
			"%w: test %#q of type %#q cannot include subtable 'tmt'",
			ErrMismatchedTestSubtable,
			testName,
			t.Type,
		)
...
func (t TestDefinition) validateLisa(testName string) error {
	if len(t.Lisa) == 0 {
		return fmt.Errorf(
			"%w: test %#q of type %#q requires a [lisa] subtable",
			ErrMissingTestField,
			testName,
			t.Type,
		)
...
func (t TestDefinition) validateTmt(testName string) error {
	if len(t.Tmt) == 0 {
		return fmt.Errorf(
			"%w: test %#q of type %#q requires a [tmt] subtable",
			ErrMissingTestField,
			testName,
			t.Type,
		)
