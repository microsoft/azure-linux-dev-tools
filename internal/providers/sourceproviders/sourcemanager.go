...
func (m *sourceManager) ResolveSourceIdentity(
	...
	case projectconfig.SpecSourceTypeLocal, projectconfig.SpecSourceTypeUnspecified:
		...
		// Local components have no upstream provenance; the %azl_upstream_* macros
		// are intentionally left unpopulated. A local spec's own identity is
		// already available at build time via %{version}/%{release}.
		return SourceIdentity{Identity: identity}, nil
